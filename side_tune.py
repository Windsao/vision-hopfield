...
from engine import train_one_epoch, evaluate, tune_one_epoch, side_evaluate
...
# def energy_contribution_loss(query, memory_set, label, pattern_per_label):

#     # query: (batch_size, hid)
#     # memory_set: (memory_size, hid)

#     energy = torch.log(torch.exp(query@memory_set.T).sum(-1)) # energy: (batch_size)

#     tgt_memory = []
#     for i in range(query.size(0)):
#         s = pattern_per_label*label
#         e = s+pattern_per_label
...
def main(args):
    ...
    if args.finetune:
        ...
        model.add_side_tuning(args.nb_classes)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats = side_evaluate(data_loader_val, model, device)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
